...
export default function page() {
  ...
  const deleteInv = async (invId) => {
    try {
      let result = await fetch(`${BaseURL}/inv/${invId}`, {
        method: "DELETE",
        headers: {
          "content-type": "application/json",
          Authorization: `${cookie}`,
      ...
      if (result.user && result?.result) {
        toast.success("Inventory is deleted!");
        FetchAllInventories();
      }
    } catch (error) {
      console.log(error);
      ...
      <div className="w-full relative overflow-y-auto overflow-x-hidden px-4 sm:px-8 pt-6 flex min-h-screen  bg-[#F7F7F7] flex-col items-center justify-start ">
        ...
        <div className="w-full flex flex-col justify-between items-center ">
          ...
          <div className="w-full flex flex-row justify-between pb-4 gap-4">
            ...
            <div className="flex flex-row gap-4">
              ...
              <button
                ...
                className="rounded bg-gradient-to-b from-[#FF8A08] to-[#FFC100] text-white text-center px-2 flex flex-row items-center justify-center gap-2 text-xs sm:text-base"
              >
          ...
          <div className="flex flex-col mt-2 min-h-auto rounded-lg bg-transparent w-full justify-center items-center">
            ...
            <hr className="w-full" />
            {inventories.length < 1 && !isLoading ? (
              <div className="my-6">No inventory created</div>
            ) : !isLoading ? (
              <div className="w-full flex flex-row flex-wrap gap-3 text-sm  justify-center py-6">
                {inventories?.map((inventory, i) => {
                  return (
                    <div
                      key={i}
                      className="rounded-lg p-4 bg-white shadow-md w-full sm:w-[40%] lg:w-[32%]"
                    >
                      <div className="w-full flex flex-row justify-between items-center mb-8">
                        ...
                        <div className="flex flex-row gap-2">
                          <BiCopy
                            ...
                            className=" p-2 hover:text-gray-800 hover:bg-gray-200 duration-200 rounded-full ease-in-out  text-3xl  cursor-pointer"
                          />
                          <MdDelete
                            onClick={() => deleteInv(inventory.ID)}
                            className=" p-2 hover:text-gray-800 hover:bg-gray-200 duration-200 rounded-full ease-in-out text-3xl cursor-pointer"
                          />
                          <FaSquareUpRight
                      ...
                      <div className="flex flex-row w-full flex-wrap gap-1">
                        <span className="px-2 py-1 text-xs bg-[#FF8A08] text-white rounded-[4px]">
                          Qty
                        </span>
                        <span className="px-2 py-1 text-xs bg-[#FF8A08] text-white rounded-[4px]">
                          Amount
